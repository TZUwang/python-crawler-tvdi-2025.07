...
from crawl4ai import (AsyncWebCrawler,
                      ...
                      SemaphoreDispatcher,  # SemaphoreDispatcher是用來限制同時運行的爬蟲數量
                      RateLimiter,          # RateLimiter是用來限制爬蟲的速率
                      )          
...
async def get_stock_data(urls)-> list[dict]:
    ...
    stock_schema = {
        ...
        "fields": [
            {
                "name":"日期時間",
                "selector":"time.last-time#lastQuoteTime",    # 假設日期時間在這個選擇器下
                "type":"text"
            },
            {
                "name": "股票號碼",
    ...
    run_config = CrawlerRunConfig(
        ...
        page_timeout=100000,  # 設定頁面超時時間（毫秒）
        #想要在`class="my-drawer-toggle-btn"`的元素上點擊  js_code=["document.querySelector('.my-drawer-toggle-btn').click();"],
    ...
    all_results:list[dict] = [] # 用來存儲所有股票資料的列表
    for result in results:
        stack_data:list[dict] = json.loads(result.extracted_content) # 將爬取到的資料轉換為字典
        all_results.append(stack_data[0]) #stack_data[0] 是因為每個結果都是一個列表，裡面只有一個字典，所以取第一個元素
