...
import asyncio
import threading #threading是用來處理多線程的模組 避免發生UI凍結的問題
import wantgoo 
...
class SimpleApp:
    ...
    def create_widgets(self): 
        ...
        self.selected_button = tk.Button(
            ...
            font=("Arial", 12, "bold"),
            state=tk.DISABLED,
            command=lambda: threading.Thread(target=self.start_crawling, daemon=True).start() #daemon=True 讓主程式結束時自動結束這個線程
        )
        ...
        # 建立 Treeview Treeview是用來顯示表格資料的元件
