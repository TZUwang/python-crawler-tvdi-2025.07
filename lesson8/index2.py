...
class SimpleApp:
    ...
    def create_widgets(self):
        ...
        self.selected_button = tk.Button(
            ...
            font=("Arial", 12, "bold"),
            state=tk.DISABLED,
            command=lambda: threading.Thread(target=self.start_crawling, daemon=True).start() #daemon=True確保主程式結束時 此線程也會結束
        )
    ...
    def start_crawling(self, event=None):
        """開始爬蟲"""
        self.selected_button.config(state=tk.DISABLED) # 禁用按鈕 避免重複點擊
        try:
            # 在這裡可以加入爬蟲邏輯
